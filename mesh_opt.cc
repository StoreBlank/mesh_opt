...
// pybinding
py::tuple normal_postprocess(py::array_t<float> vertices, py::array_t<int> faces, py::array_t<float> normals) {
    if (vertices.ndim() != 2 || vertices.shape(1) != 3)
        throw runtime_error("vertices must be a Nx3 array");
    if (faces.ndim() != 2 || faces.shape(1) != 3)
        throw runtime_error("faces must be a Nx3 array");
    if (normals.ndim() != 2 || normals.shape(1) != 3)
        throw runtime_error("normals must be a Nx3 array");
    if (vertices.shape(0) != normals.shape(0))
        throw runtime_error("vertices and normals must have the same number of rows");
    ...
    for (int i = 0; i < vertices.shape(0); i++) {
        v.push_back(point(vertices.at(i, 0), vertices.at(i, 1), vertices.at(i, 2)));
        n.push_back(vec(normals.at(i, 0), normals.at(i, 1), normals.at(i, 2)));
    }
    for (int i = 0; i < faces.shape(0); i++) {
        f.push_back(ivec3(faces.at(i, 0), faces.at(i, 1), faces.at(i, 2)));
    }
    mesh->vertices = v;
    mesh->faces = f;
    mesh->normals = n;

    optimize_mesh(mesh, 0.1, 0.1);

    py::array_t<float, py::array::c_style> new_normals({int(mesh->normals.size()), 3});
    for (unsigned int i = 0; i < mesh->normals.size(); i++) {
        new_normals.mutable_at(i, 0) = mesh->normals[i][0];
        new_normals.mutable_at(i, 1) = mesh->normals[i][1];
        new_normals.mutable_at(i, 2) = mesh->normals[i][2];
    }
    py::array_t<float, py::array::c_style> new_vertices({int(mesh->vertices.size()), 3});
    for (unsigned int i = 0; i < mesh->vertices.size(); i++) {
        new_vertices.mutable_at(i, 0) = mesh->vertices[i][0];
        new_vertices.mutable_at(i, 1) = mesh->vertices[i][1];
        new_vertices.mutable_at(i, 2) = mesh->vertices[i][2];
    }
    py::array_t<int, py::array::c_style> new_faces({int(mesh->faces.size()), 3});
    for (unsigned int i = 0; i < mesh->faces.size(); i++) {
        new_faces.mutable_at(i, 0) = mesh->faces[i][0];
        new_faces.mutable_at(i, 1) = mesh->faces[i][1];
        new_faces.mutable_at(i, 2) = mesh->faces[i][2];
    }

    delete mesh;
    return py::make_tuple(new_vertices, new_faces, new_normals);
}

PYBIND11_MODULE(mesh_opt, m) {
    m.doc() = "Mesh optimization using Efficiently Combining Positions and Normals for Precise 3D Geometry";
    m.def("normal_postprocess", &normal_postprocess, "Postprocess normals using Efficiently Combining Positions and Normals for Precise 3D Geometry",
        py::arg("vertices"), py::arg("faces"), py::arg("normals"));
}
